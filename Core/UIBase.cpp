...
void CWindowWnd::CenterWindow(bool byScreen)
{
    ASSERT(::IsWindow(m_hWnd));
...
void CWindowWnd::MaxWindow()
{
	...
	rcArea = oMonitor.rcWork;
	int w = rcArea.right - rcArea.left;
	int h = rcArea.bottom - rcArea.top;
	MoveWindow(m_hWnd, rcArea.left, rcArea.top, w, h, TRUE);
}
