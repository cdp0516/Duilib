...
namespace DuiLib
{
	...
	LRESULT CEditWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		...
		{
			m_pOwner->GetManager()->SendNotify(m_pOwner, DUI_MSGTYPE_EDIT_KEYDOWN, wParam);
	...
	LRESULT CEditWnd::OnEditChanged(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
	{
		...
		{
			std::wstring txt = pstr;
			unsigned int i = _wtoi(txt.c_str());
			if (i > m_pOwner->m_uMaxNumber)
			{
				txt = txt.substr(0, txt.length() - 1);
				m_pOwner->SetText(txt.c_str());
				m_pOwner->SetSel(txt.length(), txt.length());
				return 0;
			}
		}
	...
	CEditUI::CEditUI() : m_pWindow(NULL), m_uMaxChar(255), m_bReadOnly(false), 
		...
		m_dwEditbkColor(0xFFFFFFFF), m_dwEditTextColor(0x00000000), m_iWindowStyls(0), m_dwTipValueColor(0xFFBAC0C5), m_uMaxNumber(UINT_MAX)
	{
	...
	void CEditUI::SetMaxNumber(UINT uMax)
	{
		m_uMaxNumber = uMax;
	}
	...
	void CEditUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if (_tcsicmp(pstrName, _T("maxnumber")) == 0) SetMaxNumber(_ttoi(pstrValue));
